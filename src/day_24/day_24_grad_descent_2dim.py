...
class Hail:
    ...
    def solve(self):
        ...
        def error_function(vars):
            x, z, vx, vz = vars
            return self._error_function((x, z), (vx, vz))

        initial_guess = np.array([0, 0, 0, 5])

        max_v_module = 999

        def constraint_vx(x):
            return max_v_module - abs(x[2])
        def constraint_vz(x):
    ...
    def _error_function(self, p: Point, v: Point) -> float:
    ...
    def _get_dist_squared(self, p: Point, v: Point, stone: Stone) -> float:
        # knowing the rocket pos & speed, find the rocket's distance to each of the M stones
        # at the moments the rocket hits them
        max_error = 99999999999999999999999
        t = (stone.p[1] - p[1]) / (v[1] - stone.v[1])
