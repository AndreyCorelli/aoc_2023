...
class Hail:
    ...
    def solve(self):
        ...
        for i in range(50):
            # N random points to try
            gd = GradDescend(error_function)
            error, point = gd.solve(
                2000, start_lr=2, end_lr=0.1
            )
            if error < min_error:
                min_error = error
                best_point = point
                coords = self._find_point(best_point)
                print(f"New min {min_error} at v={best_point}, coords={coords}, iteration={i}")
...
class GradDescend:
    ...
    def solve(self,
              max_iter: int,
              start_lr: float = 1,
              end_lr: float = 0.08) -> Tuple[float, Point3]:
        self.errors: List[float] = []
        self.vx_lst: List[float] = []
        ...
        for iteration in range(max_iter):
            ...
            learning_rate = start_lr + (end_lr - start_lr) * k
            ...
            for i in range(3):
                ...
                gradient.append(move * learning_rate)

            new_point = [point[i] - gradient[i] for i in range(3)]
            point = tuple([int(round(pi)) for pi in new_point])

            current_error = self.error_function(point)

            self.errors.append(current_error)
            self.vx_lst.append(point[0])
    ...
    def _get_random_point(self) -> Point3:
        return tuple([
    ...
    def _get_neighbours(self, p: Point3) -> List[Point3]:
        nbs = []
        while len(nbs) < 8:
            p2 = p
            for j in range(6):
