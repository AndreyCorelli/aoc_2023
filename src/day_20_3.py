from typing import TypeAlias, Dict, List, Optional, Tuple, Set
...
class Circuit:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.module_period: Dict[str, Set[int]] = {c: set() for c in self.LAST_CASCADE}
        self.ticks = 0
    ...
    def tick(self):
        ...
        while signals_to_pass:
            ...
            last_outputs = {self.modules[m].name: self.modules[m].last_output for m in self.LAST_CASCADE}
            for key, value in last_outputs.items():
                if value == HIGH:
                    self.module_period[key].add(self.ticks)
...
def main():
    ...
    print(circuit.module_period)
    # {'qz': {19555, 3911, 7822, 11733, 15644}, 'cq': {8042, 16084, 4021, 12063}, 'jx': {3907, 7814, 11721, 15628, 19535}, 'tt': {19655, 15724, 11793, 7862, 3931}}

    for name, values in circuit.module_period.items():
        values = list(values)
        values.sort()
        print(f"{name}: {values[-1] - values[-2]}")
