...
class Node:
    ...
    def __repr__(self):
        ...
        return f"{self.point} ({self.weight}). Neighbours: {nbs}"
...
class Graph:
    ...
    def solve(self) -> int:
        self._build_nodes()
        # find the longest path from start to end using Dijkstra
        start = [n for n in self.nodes if n.point == (1, 0)][0]
        end = [n for n in self.nodes if n.point == (self.w - 2, self.h - 1)][0]
        self.dijkstra(start)
        result = -end.weight
    ...
    def _build_graph(self, prev_node: Point, start: Point, visited: Set[Point]):
        ...
        while True:
            ...
            for nxt in next_points:
                self._build_graph(cur, nxt, visited)
            break
        return
...
def small_run():
    island = Island("input_d23_small.txt")
    island.solve()
    island.print()
...
- Graph is not OK: edge from (13, 13) -> (21, 11) not found :(
- Dijkstra is probably OK
"""